...
async function playOnlineTTSWindows(text, langCode) {
    try {
        ...
        const psScript = [
            ...
            `    $player.Play()`,
            `    `,
            `    # Piso minimo de 200ms de "wake": em velocidades > 1x o resampler de audio`,
            `    # precisa de um tempo real minimo pra se acomodar ao novo SpeedRatio antes`,
            `    # de tocarmos o trecho que o usuario realmente vai ouvir. Sem esse piso,`,
            `    # frases curtas nao davam tempo suficiente e a primeira palavra saia cortada.`,
            `    $minWakeMs = 200`,
            `    $maxWakeMs = [Math]::Max($minWakeMs, [Math]::Min(2000, [int]($realDuration * 0.3)))`,
            `    $wakeElapsedMs = 0`,
            `    while ($player.Position.TotalMilliseconds -eq 0 -and $wakeElapsedMs -lt $maxWakeMs) {`,
            `        Start-Sleep -m 50`,
            `        $wakeElapsedMs += 50`,
            `    }`,
            `    # Garante que o piso minimo de acomodacao do resampler seja respeitado`,
            `    # mesmo que a posicao ja tenha avancado antes do fim do loop acima`,
            `    if ($wakeElapsedMs -lt $minWakeMs) {`,
            `        Start-Sleep -m ($minWakeMs - $wakeElapsedMs)`,
            `    }`,
            `    `,
            `    # Retrocede para o inicio da pronuncia com a placa ja ativa e o`,
            `    # resampler ja estabilizado na velocidade alvo`,
            `    $player.Position = New-Object System.TimeSpan(0)`,
            `    `,
            `    # Pequena pausa para o pipeline de audio descarregar o buffer antigo`,
            `    # antes do replay "real" - evita a gaguejada na primeira palavra`,
            `    Start-Sleep -m 80`,
            `    `,
            `    # Forca o replay: se o clipe ja tiver chegado ao fim durante a fase de wake`,
            `    # (comum em frases curtas + velocidade alta), o player para sozinho e o`,
            `    # reset de Position acima nao reinicia a reproducao automaticamente.`,
            `    $player.Play()`,
            `    `,
            `    # Aguarda o tempo de reproducao completo (ja ajustado pela velocidade) + margem de seguranca`,
            `    $sleepTime = [int]$realDuration + 300`,
